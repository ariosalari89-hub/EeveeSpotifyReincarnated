...
try {
    ...
    foreach($phase in $phases) {
        $height=if($phase-eq'inline'-or$phase.EndsWith('-inline')){52}elseif($phase-eq'background'-or$phase.EndsWith('-fullscreen')){640}elseif($phase-eq'card-layout'){392}else{320}
