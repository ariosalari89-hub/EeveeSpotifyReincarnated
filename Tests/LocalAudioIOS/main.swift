...
final class QAAppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func run(navigation: UINavigationController) async {
        do {
            ...
            try await verifyNativeRenameRecovery(navigation: navigation)
            try await verifyNativeRemovalConfirmation(navigation: navigation)
